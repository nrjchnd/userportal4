define(function(require){
	...
	var app = {
		...
		bindReceivedVMs: function(template) {
			...
			template.find('#refresh_voicemails').val(function() {
				...
				self.usersGetUser(userId, function(user) {
						//console.log(user);
					self.getVmboxByOwner(user.id, function(vmbox) {
						//console.log(vmbox);
						var vmboxId = vmbox[0].id;
		...
		getVmboxByOwner: function(userId, callback) {
                var self = this;
                self.callApi({
                    resource: "voicemail.list",
                    data: {
                    ...
                    success: function(voicemail) {
                        callback && callback(voicemail.data)
                    }
                })
            },

        accountGet: function(accountId, callback) {
        	var self = this;
        	...
        	self.callApi({
				...
				success: function(account) {
					callback && callback(account.data);
				}
			});
        },
		...
		getVmboxId: function(userId, callback) {
			var self = this;
